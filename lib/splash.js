...
export default function splash ({ path, name, org, coc, logo, active, total, channels, large, iframe, expressFlash }){
  let div = dom('.splash',
    !iframe && dom('.logos',
      logo && dom('.logo.org'),
      dom('.logo.slack')
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' 人がログイン中です。',
          dom('b.total', total), ' が登録されています。'
        ]
        : [dom('b.total', total), ' 人が近々登録されました。']
    ),
    dom('form id=invite action="/invite" method="POST"',
      channels && (
        channels.length > 1
  ...
  //    ),
      expressFlash && (
		      expressFlash.length > 0
                      ? dom('button.loading disabled=1',expressFlash) : dom('button.loading', 'JOIN する。')
		      ),
    ),
    dom('p.signin',
      'or ',
